...
### 4.2.2 XGboost Model
Due to the lack of historical data, the dataset is biassed. Therefore, the XGboost model is used for the rent price prediction, referring to the `/models/rent_price_xgboost.ipynb`. Using the holdout method for train test split and forward selection with lowest Mean Squared Error, the XGboost regression model is trained, and the training accuracy is 0.742 while the test accuracy is 0.697. A feature importance graph is also plotted by XGboost built-in function. Further, this notebook predicts the rent prices for 2023 - 2027.
